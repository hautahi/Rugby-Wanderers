...
Much of the debate about player poaching is predicated on the assumption that there has been an increase in the number of foreign born players. The graph below, which presents the number of foreign players making their debut in each year for the 10 major nations, shows that, on the contrary, there is no noticeable trend in the number of foreign players. It has generally fluctuated around an average of `r round(mean(tl$Foreign))` foreign-born debutants per year, with a low of `r min(tl$Foreign)` in `r tl$Debut[which.min(tl$Foreign)]` and a high of `r max(tl$Foreign)` in `r tl$Debut[which.max(tl$Foreign)]`. There were more foreign born players in 2002, for example, than there were in 2015.
...
That no countries are immune to player poaching is not surprising. International migration is a reality of the modern world, and many of the top rugby countries have large foreign born populations. The following graph compares the extent to which the foreign born composition of each rugby side is representative of the general population in that country.^[I use 2010 census data retrieved from the [World Bank Data Bank](http://databank.worldbank.org/data/reports.aspx?source=2&country=ARG&series=&period=#advancedDownloadOptions) (Accessed on 25 March 2016). The dataset is [here](https://github.com/hautahi/Rugby-Wanderers/blob/master/data/WorldBankData/Data.csv) and details of the definitions and sources for each country can be found [here](https://github.com/hautahi/Rugby-Wanderers/blob/master/data/WorldBankData/DefinitionandSource.csv). I do not have separate migration data for the British countries. I therefore use the total United Kingdom figure of `r m1[m1$Team=="England" & m1$variable =="Foreign born Population (%)",3]`% for each of England, Scotland and Wales.] The red bars repeat the foreign born player percentages from the graph above while the blue bars show the percentage of each country’s population that was not born in that country. `r WB$Team[1]` has the highest proportion of its population born overseas, at `r WB$value[1]`%, followed by `r WB$Team[2]` with `r WB$value[2]`% and `r WB$Team[3]` with `r WB$value[3]`%.  By comparing the each country's respective blue and red bars, we see that only New Zealand and Argentina have rugby teams which *under*-represent the foreign-born population. Foreign-born players are *over*-represented for every other country.
...
The next table summarises the net contribution of each country to the world rugby stock. The "trade balance" is calculated by subtracting the number of "imported" players from the number of "exported" players. A positive trade balance indicates that a country contributes more players to other countries than it "poaches". Leading the way is `r Trade$Country[1]`, which has provided `r Trade[1,4]` more players to other countries than it has poached. On the other side of the ledger is `r Trade$Country[length(Trade$Country)]`, which has imported `r -Trade$"Trade Balance"[Trade$Country=="Scotland"]` more players than it has exported.
