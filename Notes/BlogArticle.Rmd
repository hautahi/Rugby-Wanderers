...
The prevalence of Pacific born players in foreign teams has garnered particular interest with both the rugby public and media, although just `r nrow(PI_df)` of the top 10 major countries have had more than two players born in the Pacific Island nations of Samoa, Fiji, and Tonga since 1996.^[While acknowledging the problematic nature of collectively labelling the diverse communities within the South Pacific, for the purposes of this discussion "the Pacific" refers to the countries of Samoa, Fiji and Tonga - the three Pacific Island countries with the largest rugby playing populations.] `r PI_df[PI_df$Team=="Aus",2][[1]]` Wallabies, `r PI_df[PI_df$Team=="Eng",2][[1]]` English players and `r PI_df[PI_df$Team=="NZ",2][[1]]` All Blacks were born in the Pacific. However, these numbers do not tell the full story as each country has vastly different Pacific born populations. `r PI_df[PI_df$Team=="NZ",9][[1]]`% of the New Zealand population were born in the Pacific Islands, compared to just `r PI_df[PI_df$Team=="Aus",9][[1]]`% in Australia and less than 0.1% in England.^[These figures are calculated using 2015 [United Nations](http://www.un.org/en/development/desa/population/migration/data/estimates2/data/UN_MigrantStockByOriginAndDestination_2015.xlsx) and [World Bank](http://data.worldbank.org/data-catalog/Population-ranking-table) data (Accessed on 22 April 2017).] Pacific born athletes are wonderful rugby players, and it is therefore unsurprising that they are *over*-represented in all these teams. However, the extent to which they are over represented differs markedly across teams. While New Zealand has `r ceiling(PI_df[PI_df$Team=="NZ",10][[1]])` times more Pacific born players than would be expected given the population, Australia has `r ceiling(PI_df[PI_df$Team=="Aus",10][[1]])` times more, and England a whopping `r ceiling(PI_df[PI_df$Team=="Eng",10][[1]])` times more.
...
The final table addresses a commonly argued point that without imports, some countries would not compete at the same level. The figures below show that imports tend not to differ too much from their native born teammates in terms of contributions on the field. For example, only Ireland, Italy, and South Africa have imports that score more points on average than their native-born counterparts, although these figures are skewed by foreign-born kickers such as Ronan O'Gara and Percy Montgomery. Foreign players, however, tend to score more tries, on average, than their native born team mates, the exceptions being Australia, England and Ireland.
