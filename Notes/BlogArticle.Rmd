...
The following table presents an overview of players to debut in the professional era.^[I am missing the birth information for `r fin_df2 %>% filter(Team %in% target) %>% select(MissPlayers) %>% sum()` of these players, and they are not included in this table. `r fin_df2$MissPlayers[fin_df2$Team=="Argentina"]` of the missing players are from Argentina, `r fin_df2$MissPlayers[fin_df2$Team=="Ireland"]` from Ireland, `r fin_df2$MissPlayers[fin_df2$Team=="France"]` from France and `r fin_df2$MissPlayers[fin_df2$Team=="Italy"]` from Italy.] On average, `r round(mean(FB$TotalPlayers))` players debuted for each country since 1996, ranging from `r min(FB$TotalPlayers)` for `r FB$Team[which.min(FB$TotalPlayers)]` to `r max(FB$TotalPlayers)` for `r FB$Team[which.max(FB$TotalPlayers)]`.
