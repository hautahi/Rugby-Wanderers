...
Much of the debate about player poaching is predicated on the assumption of an increasing trend in the number of foreign born players. The graph below presents the number of foreign players making their debut in each year for the 10 major nations. It shows that, on the contrary, there is no noticeable trend in the number of foreign players. It has generally fluctuated around an average of `r round(mean(tl$Foreign))` foreign-born debutants per year, with a low of `r min(tl$Foreign)` in `r tl$Debut[which.min(tl$Foreign)]` and a high of `r max(tl$Foreign)` in `r tl$Debut[which.max(tl$Foreign)]`. There were more foreign born players in 2002, for example, than there were in 2015.
